...
namespace ARPGSimpleDemo.Skill
{
	class SectorAttackSelector : IAttackSelector
	{
        ...
        {
            var ownerTrans = weaponInfo.owner;
            //发一个球形射线，找出所有碰撞体
            var colliders = Physics.OverlapSphere(ownerTrans.position, weaponInfo.distance);
            ...
            array = CollectionHelper.FindAll<GameObject>(array,
                 p => 
                    Array.IndexOf(weaponInfo.selectorTags, p.tag) >= 0 &&
                    p.GetComponent<CharaController>() != null && p.GetComponent<CharacterInfo>().HP > 0 && 
                    Vector3.Angle(ownerTrans.forward,p.transform.position - ownerTrans.position) <= weaponInfo.attackAngle /2 
                    );
